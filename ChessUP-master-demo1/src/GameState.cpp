    ...
    while (gameState_ == GameMode::GAME_MODE_PVP)
    {
        ...
        while (SDL_PollEvent(&e))
        {
            ...
            else if(e.type == SDL_KEYDOWN)
            {
                ...
                case SDLK_z:
                    states->undoMove();
                    states->setPieceTurn(!states->getPieceTurn());
                    break;
                default:
                    break;
                }
        ...
        if(board->getFocusedPiece() != nullptr)
        {
            ...
            if((board->getFocusedPiece()->getName() != PieceName::EMPTY) && (board->getFocusedPiece()->getColor() == states->getPieceTurn()))
            {
            ...
            if(board->getFocusedPiece()->getColor() == states->getPieceTurn())
        ...
        {
            // Render endgame
            t_EndGame[(int) gameResult].render(board->getXboard(), board->getYboard(), CELL_SIZE * 8, CELL_SIZE * 8);
...
void GameState::renderEditMode()
{
    ...
    board->editBoard(this,states);
    states->setPieceTurn(board->choosePieceTurn(this, states));

    while(gameState_ == GameMode::GAME_MODE_EDIT)
    {
        // Handles the event queue
        while (SDL_PollEvent(&e))
        {
        ...
        if(gameState_ == GameMode::GAME_MODE_SAVE)
        {
            ...
            gameState_ = GameMode::GAME_MODE_EDIT;
        }
